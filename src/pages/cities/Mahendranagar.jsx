...
const Mahendranagar = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if Mahendranagar is in favorites
        const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
        setIsFavorite(favorites.includes('Mahendranagar'));
      } catch (error) {
  ...
  const toggleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
    if (isFavorite) {
      const updatedFavorites = favorites.filter(city => city !== 'Mahendranagar');
      localStorage.setItem('favoriteCities', JSON.stringify(updatedFavorites));
    } else {
      favorites.push('Mahendranagar');
  ...
  const getPollutionSources = () => {
    return {
      mainSources: ["Dust from dry terrain", "Agricultural burning", "Vehicle emissions", "Cross-border pollution"],
      description: "Mahendranagar experiences air quality challenges primarily due to its location in the dry far-western Terai region. Dust from unpaved roads, agricultural activities, and pollution drifting across the border from India contribute to the city's air quality issues. The hot climate exacerbates ozone formation during summer months."
    };
  };
  ...
  if (loading) {
    return <div className="loading-container">Loading Mahendranagar's air quality data...</div>;
  }
  ...
  return (
    <>
      <div className="mahendranagar-container">
        <div className="content-wrapper">
          <div className="header-section">
            <h1 className="head">Mahendranagar Air Quality</h1>
            <button 
          ...
          <div className="weather-section">
            <h2 className="headerr">Mahendranagar Weather</h2>
          ...
          <div className="sources-section">
            <h2 className="headerr">Pollution Sources in Mahendranagar</h2>
