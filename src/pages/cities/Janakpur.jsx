...
const Janakpur = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if Janakpur is in favorites
        const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
        setIsFavorite(favorites.includes('Janakpur'));
      } catch (error) {
  ...
  const toggleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
    if (isFavorite) {
      const updatedFavorites = favorites.filter(city => city !== 'Janakpur');
      localStorage.setItem('favoriteCities', JSON.stringify(updatedFavorites));
    } else {
      favorites.push('Janakpur');
  ...
  const getPollutionSources = () => {
    return {
      mainSources: ["Religious tourism traffic", "Agricultural burning", "Domestic fuel burning", "Dust from unpaved roads"],
      description: "Janakpur, as a major religious and cultural center, faces air quality challenges from increasing pilgrimage traffic and urban development. The city's location in the central Terai makes it susceptible to seasonal dust storms and agricultural burning. Traditional cooking methods in many households also contribute to local pollution."
    };
  };
  ...
  if (loading) {
    return <div className="loading-container">Loading Janakpur's air quality data...</div>;
  }
  ...
  return (
    <>
      <div className="janakpur-container">
        <div className="content-wrapper">
          <div className="header-section">
            <h1 className="head">Janakpur Air Quality</h1>
            <button 
          ...
          <div className="weather-section">
            <h2 className="headerr">Janakpur Weather</h2>
          ...
          <div className="sources-section">
            <h2 className="headerr">Pollution Sources in Janakpur</h2>
