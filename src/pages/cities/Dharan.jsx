...
const Dharan = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if Dharan is in favorites
        const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
        setIsFavorite(favorites.includes('Dharan'));
      } catch (error) {
  ...
  const toggleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
    if (isFavorite) {
      const updatedFavorites = favorites.filter(city => city !== 'Dharan');
      localStorage.setItem('favoriteCities', JSON.stringify(updatedFavorites));
    } else {
      favorites.push('Dharan');
  ...
  const getPollutionSources = () => {
    return {
      mainSources: ["Vehicle emissions", "Construction activities", "Biomass burning", "Industrial emissions"],
      description: "Dharan faces air quality challenges from rapid urbanization and its location at the foothills of the Himalayas. While generally better than Terai cities, increasing vehicular traffic and construction activities contribute to pollution. The city's elevation helps with some dispersion of pollutants."
    };
  };
  ...
  if (loading) {
    return <div className="loading-container">Loading Dharan's air quality data...</div>;
  }
  ...
  return (
    <>
      <div className="dharan-container">
        <div className="content-wrapper">
          <div className="header-section">
            <h1 className="head">Dharan Air Quality</h1>
            <button 
          ...
          <div className="weather-section">
            <h2 className="headerr">Dharan Weather</h2>
          ...
          <div className="sources-section">
            <h2 className="headerr">Pollution Sources in Dharan</h2>
