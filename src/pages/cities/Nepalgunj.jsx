...
const Nepalgunj = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if Nepalgunj is in favorites
        const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
        setIsFavorite(favorites.includes('Nepalgunj'));
      } catch (error) {
  ...
  const toggleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
    if (isFavorite) {
      const updatedFavorites = favorites.filter(city => city !== 'Nepalgunj');
      localStorage.setItem('favoriteCities', JSON.stringify(updatedFavorites));
    } else {
      favorites.push('Nepalgunj');
  ...
  const getPollutionSources = () => {
    return {
      mainSources: ["Vehicle emissions", "Industrial activities", "Brick kilns", "Dust from dry terrain"],
      description: "Nepalgunj faces air quality challenges due to its industrial activities, increasing vehicular traffic, and its location in the dry Terai region where dust is prevalent. The city's proximity to the Indian border also contributes to cross-border pollution."
    };
  };
  ...
  if (loading) {
    return <div className="loading-container">Loading Nepalgunj's air quality data...</div>;
  }
  ...
  return (
    <>
      <div className="nepalgunj-container">
        <div className="content-wrapper">
          <div className="header-section">
            <h1 className="head">Nepalgunj Air Quality</h1>
            <button 
          ...
          <div className="weather-section">
            <h2 className="headerr">Nepalgunj Weather</h2>
          ...
          <div className="sources-section">
            <h2 className="headerr">Pollution Sources in Nepalgunj</h2>
