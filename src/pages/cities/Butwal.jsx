...
const Butwal = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Check if Butwal is in favorites
        const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
        setIsFavorite(favorites.includes('Butwal'));
      } catch (error) {
  ...
  const toggleFavorite = () => {
    const favorites = JSON.parse(localStorage.getItem('favoriteCities')) || [];
    if (isFavorite) {
      const updatedFavorites = favorites.filter(city => city !== 'Butwal');
      localStorage.setItem('favoriteCities', JSON.stringify(updatedFavorites));
    } else {
      favorites.push('Butwal');
  ...
  const getPollutionSources = () => {
    return {
      mainSources: ["Vehicle emissions", "Industrial activities", "Road construction", "Biomass burning"],
      description: "Butwal's air quality is affected by its growing industrial sector and increasing vehicular traffic. The city's location in the Terai foothills means it experiences both valley pollution effects and some dispersion benefits. Road expansion projects and traditional cooking methods also contribute to local air pollution."
    };
  };
  ...
  if (loading) {
    return <div className="loading-container">Loading Butwal's air quality data...</div>;
  }
  ...
  return (
    <>
      <div className="butwal-container">
        <div className="content-wrapper">
          <div className="header-section">
            <h1 className="head">Butwal Air Quality</h1>
            <button 
          ...
          <div className="weather-section">
            <h2 className="headerr">Butwal Weather</h2>
          ...
          <div className="sources-section">
            <h2 className="headerr">Pollution Sources in Butwal</h2>
